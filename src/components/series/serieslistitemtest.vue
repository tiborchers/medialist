...
export default {
  name: 'SeriesListItem',
  components: {
    'EpisodeTable': () => ({ component: import('./eptable.vue'), loading: import('./spinner.vue') })
  },
  ...
  mounted () {
    this.getProgressasync(this.series)
  },
  methods: {
    ...
    getProgressasync: async function (series) {
      axios.get('http://192.168.1.90:3000/api/series/' + series.id + '/status')
        .then(response => {
    ...
    markAsWatchedEpisode: function (episode) {
      axios.patch('http://192.168.1.90:3000/api/episode/' + episode.id + '/watched')
        .then(async () => {
          await this.getProgressasync(this.series)
          this.$emit('modified')
        })
    },
