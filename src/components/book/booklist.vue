    ...
    <div v-if="isOnline">
      ...
      <div class="creation">
        <b-form-input
          v-model="url"
          type="url"
          placeholder="Ingrese URL de GoodReads"
          @keyup.enter.native="createBook()"
        />
        <b-button
          v-if="loading"
          disabled
...
export default {
  ...
  methods: {
    init: function () {
      this.loading = true
      if (this.isOnline) {
        axios.get('http://192.168.1.90:3000/api/books/toread')
          .then(response => {
            this.loading = false
            this.books = response.data
            this.allBooks = this.books
            this.$offlineStorage.set('booksToRead', response.data)
            this.initPages()
          })
      } else {
        this.loading = false
        this.books = this.$offlineStorage.get('booksToRead')
        this.allBooks = this.books
        this.initPages()
      }
    },
    ...
    compareByAuthor: function (a, b) {
      const genreA = a.Book.author.toUpperCase()
      const genreB = b.Album.author.toUpperCase()
    ...
    createBook: function () {
      this.loading = true
      axios.post('http://192.168.1.90:3000/api/books/url', { 'url': this.url })
        .then(response => {
          this.url = ''
          this.error = ''
          this.init()
    ...
    getRandom: function () {
      ...
      this.filterbyGenre()
      this.filterbyHLTR()
      this.books = [this.books[Math.floor(Math.random() * this.books.length)]]
      this.initPages()
    },
