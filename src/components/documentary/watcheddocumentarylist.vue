<template>
  <div class="main">
    ...
    <div v-if="isOnline">
      <div
        v-for="(documentary, index) in documentaries"
        :key="documentary.id.toString()"
      >
        <WatchedDocumentaryListItem
...
import WatchedDocumentaryListItem from './watcheddocumentarylistitem.vue'
...
export default {
  ...
  mounted () {
    ...
    if (this.isOnline) {
      ...
      this.loading = false
      this.documentaries = this.$offlineStorage.get('documentariesWatched')
      this.allDocumentaries = this.documentaries
      this.initPages()
    }
    this.getGenres()
  ...
  methods: {
      ...
      if (this.isOnline) {
        axios.get('http://192.168.1.90:3000/api/genres/documentaries')
          .then(response => {
            ...
            this.$offlineStorage.set('documentariesGenres', response.data)
          })
      } else {
    ...
    sortbyTitle: function () {
      this.documentaries.data = this.documentaries.sort(this.compareByName)
      this.initPages()
    },
    sortbyYear: function () {
      this.documentaries.data = this.documentaries.sort(this.compareByYear)
      this.initPages()
    },
    sortbyDuration: function () {
      this.documentaries.data = this.documentaries.sort(this.compareByDuration)
      this.initPages()
    },
    sortbyRating: function () {
      this.documentaries.data = this.documentaries.sort(this.compareByRating)
      this.initPages()
    },
    rsortbyTitle: function () {
      this.documentaries.data = this.documentaries.sort(this.compareByName).reverse()
      this.initPages()
    },
    rsortbyYear: function () {
      this.documentaries.data = this.documentaries.sort(this.compareByYear).reverse()
      this.initPages()
    },
    rsortbyDuration: function () {
      this.documentaries.data = this.documentaries.sort(this.compareByDuration).reverse()
      this.initPages()
    },
    rsortbyRating: function () {
      this.documentaries.data = this.documentaries.sort(this.compareByRating).reverse()
      this.initPages()
    },
