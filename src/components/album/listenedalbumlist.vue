...
import ListenedAlbumListItem from './listenedalbumlistitem.vue'
import SimpleAlbumListItem from './simplealbumlistitem.vue'
import InfiniteLoading from 'vue-infinite-loading'

export default {
  name: 'ListenedAlbumGameList',
  components: {
    ListenedAlbumListItem,
    SimpleAlbumListItem,
    InfiniteLoading
  ...
  methods: {
    init: function () {
      ...
      if (this.isOnline) {
        ...
        this.loading = false
        this.albums = this.$offlineStorage.get('albumsListened')
        this.allAlbums = this.albums
        this.initPages()
      }
    },
    ...
    filterbyText: function () {
      let ver = this.albums.map(album => {
        if (album.title.toLowerCase().includes(this.filterText.toLowerCase())) {
          return album
        }
      })
    ...
    filterbyHLTB: function () {
      if (this.filterHLTB.exact) {
        let ver = this.albums.map(album => {
          if (album.Albums.duration === this.filterHLTB.exact) {
            return album
          }
        })
        ver = ver.filter(function (element) {
